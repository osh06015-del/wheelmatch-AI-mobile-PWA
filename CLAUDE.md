...
| E2E 흐름      | `npx vitest run src/e2e` (happy-dom. `npm test`에도 포함) |
...
- 사용자에게 보이는 문구는 `src/lib/i18n/messages/ko.ts`(원본 한국어)에 키로 넣고
  나머지 4개 언어를 함께 채운다. 컴포넌트에 한국어를 직접 쓰지 않는다
  (`noHardcodedKorean.test.ts`가 막는다). 엔진 사유의 조사(을/를)는 `withParticle()`을 쓴다.
