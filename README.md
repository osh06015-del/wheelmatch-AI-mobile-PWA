...
`src/lib/rules/engine.ts`에 12종이 있다. 어느 규칙도 건너뛰지 않고, 항목마다 결과를 남긴다.
...
| RPM 안전          | 숫돌 `maxRPM` < 그라인더 `noLoadRPM`          | 둘 중 하나가 없음                                                                                           |
...
| 숫돌 종류         | —                                             | 일반 결합숫돌로 확인되지 않음 — 다른 종류(다이아몬드·플랩디스크·컵휠·와이어 브러시·기타)이거나 **모르겠음** |
...
숫돌 종류는 경고가 아니다. 작업자가 일반 결합숫돌로 확인한 경우에만 통과하고, 다른 종류와
**모르겠음**은 판정불가로 막는다. AI가 사진으로 본 종류는 확인 화면의 제안값일 뿐이다.
