...
//
// **이 모듈이 낸 값으로 품질을 경고하거나 촬영을 막지 않는다.** 그런 판단은
// 이 앱의 설계에 없다(researchMode.ts와 같은 원칙 — 서술 통계일 뿐이다).
// 그래서 아래 두 경계값도 판정 근거가 아니라 "얼마나 어둡다/밝다"를 세는
// 기준일 뿐이며, 바뀌어도 점검 흐름에는 아무 영향이 없다.
//
