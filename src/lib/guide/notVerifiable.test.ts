...
describe('확인할 수 없는 것 목록', () => {
  it('네 가지를 모두 적어 둔다', () => {
    expect(NOT_VERIFIABLE.map((item) => ko[item.titleKey])).toEqual([
      '내부 균열',
      '물리적 손상',
      '올바른 장착',
      '방호덮개 상태',
  ...
  it('각 항목이 지시문으로 끝난다', () => {
    ...
    for (const item of NOT_VERIFIABLE) {
      expect(ko[item.detailKey].length).toBeGreaterThan(20);
      expect(ko[item.detailKey]).toMatch(/세요\.$/);
    }
  });
  ...
  it('"안전하다"고 말하지 않는다', () => {
    ...
    for (const item of NOT_VERIFIABLE) {
      expect(ko[item.detailKey]).not.toMatch(
        /안전합니다|이상 없습니다|괜찮습니다/,
      );
    }
  });
